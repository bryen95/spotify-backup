...
def main():
    ...
    parser.add_argument('--format', choices=['json', 'txt', 'csv'], help='output format')
    parser.add_argument('file', help='output filename', nargs='?')
    args = parser.parse_args()

    log("Welcome to Spotify Backup!" + '\r\n')

    print(os.getcwd())
    while not args.format:
        file_format = input('Enter the format you want for the output file (Type json for a JSON dump, txt for a text '
                            'file or csv for a CSV file): ')
        if file_format in ['json', 'txt', 'csv']:
            args.format = file_format

    # If they didn't give a filename, then just prompt them. (They probably just double-clicked.)
    while not args.file:
        if args.format == 'csv':
            args.file = input('Enter a name for the output folder: ')
        else:
            args.file = input('Enter a name for the output file (e.g. my-playlist-backup.txt): ')
    ...
    if args.format == 'json':
        ...
        Path(os.getcwd() + '/' + args.file).mkdir(parents=True, exist_ok=True)
        os.chdir(os.getcwd() + '/' + args.file)
        for playlist in playlists:
            with open(clean_filename(playlist['name'][:50]) + '.csv', 'w', newline='', encoding='utf-8') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(["Title", "Artists", "Album", "SpotifyURI"])  # Header row
                for track in playlist['tracks']:
                    title = track['track']['name']
                    artists = ', '.join([artist['name'] for artist in track['track']['artists']])
                    album = track['track']['album']['name']
                    uri = track['track']['uri']
                    writer.writerow([title, artists, album, uri])

    log('Successfully wrote backup file: ' + args.file)
    log('Adios m8! :)')
    os.system('pause')
